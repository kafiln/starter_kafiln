...
type Token = {
  content: string;
  htmlStyle?: string | React.CSSProperties;
};

async function tokenizeCode(children: string, language: string) {
...
const HighlightedPre = React.memo(
  ({ children, language, ...props }: HighlightedPreProps) => {
    const [tokens, setTokens] = useState<null | Token[][]>(null);

    useEffect(() => {
      let isMounted = true;

      tokenizeCode(children, language).then((result) => {
        if (isMounted) {
          setTokens(result as Token[][]);
        }
      });
